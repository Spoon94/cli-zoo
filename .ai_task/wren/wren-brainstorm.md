...
- **`fmt` 半边界差**（轮 12 复核确认）：`n % 1000 == 500` 时 JS `Math.round` half-up 与 Python
  `round` half-even 差 1K（如 `998_500`：py 渲染 `998K`、pi 渲染 `999K`）。±1K 的展示差异，不修。
- **`compact_triggers` 死状态**（`wren.py`）：解包后从不使用，只写进缓存文件。清理级。
- **`scan_transcript` 覆盖缺口 + 缓存无完整性锚点**（轮 12 定性从「测试债」升级为「测试债 + 低可达缺陷」）：
  缓存有效性只校验 `path` 与 `0 <= offset <= size`，同路径下等长内容替换会复用陈旧 offset 静默给错数
  （轮 12 判别器实测：写入 111 → 等长覆盖 222 → 仍显示 111）。CC transcript 路径带会话 UUID 且实际只追加，
  可达性低，不阻塞合入。后续修法：缓存里加 size/mtime/首 N 字节哈希锚点，并补等长覆盖判别用例。
- **`wren.ts` 的 `git.repo` 死状态**：赋值后从不读取（轮 12 发现）。清理级。
- **`first_ts` 兜底路径无下界**：transcript 首条时间戳在未来（时钟偏差/重放）时渲染负数时长
  （轮 12 实测 `· -330m`）。真实 CC 负载走 `cost.total_duration_ms > 0` 优先，不触发。
- **`~/.cache/wren` 无 prune**：每会话一个 `<sha1[:16]>.json`，持续增长，卸载不清理
  （已在 `.test_task` T26 说明中写明边界）。
